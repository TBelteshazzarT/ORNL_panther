...
// Constructor
//
RoboClaw::RoboClaw(std::string port_name, uint32_t baud)
	:si_(port_name, baud, 5)
{
	// port.open(port_name);
...
{
	// asio::write(port, asio::buffer(&byte, sizeof(byte)));
	...
	if(si_.write(buffer)) { return sizeof(byte); }
	else { return 0; }
}
/*
int RoboClaw::read()
		...
		if(data!=-1){
			...
			if(data!=-1){
				...
				if(data!=-1){
					...
					if(crc_get()==ccrc){
						*valid = true;
						// std::cout << "Roboclaw read success\n";
						return value;
					}
				}
			}
